...
import { getSales, addSale as addSaleAction, deleteSale as deleteSaleAction } from "./actions";
...
export default function Home() {
  ...
  const filtered = entries.filter((e) => isDateInRange(e.date, start, end));
      ...
      <div className="mx-auto max-w-2xl px-5 py-8 sm:px-8 sm:py-10">
        ...
        <section className="rounded-2xl border-2 border-[var(--border)] bg-white p-5 shadow-sm sm:p-7">
          <div className="mb-5 flex flex-col gap-2 sm:mb-6 sm:flex-row sm:items-baseline sm:justify-between">
            <span className="text-lg text-[var(--foreground-muted)] sm:text-xl">{FILTER_LABELS[filter]}</span>
          ...
          {filtered.length === 0 ? (
            <p className="py-10 text-center text-lg text-[var(--foreground-muted)] sm:text-xl">
              No sales in this period. Add a sale above to get started.
            </p>
          ) : (
            <ul className="divide-y-2 divide-[var(--border)]">
              {filtered.map((entry) => (
                <li
                  key={entry.id}
                  className="flex items-center justify-between gap-4 py-4 first:pt-0 sm:py-5"
                >
                  ...
                  <button
                    ...
                    disabled={actionPending}
                    className="shrink-0 rounded-xl p-3 text-[var(--foreground-muted)] transition hover:bg-red-100 hover:text-red-600 disabled:opacity-50 min-h-[48px] min-w-[48px] flex items-center justify-center"
